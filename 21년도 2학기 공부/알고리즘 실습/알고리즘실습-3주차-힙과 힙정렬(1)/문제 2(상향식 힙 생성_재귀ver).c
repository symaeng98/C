...
//함수 선언
void recursiveBuildHeap(H *heap, int index);
...
void main(){
    ...
    recursiveBuildHeap(h, 1);
    printHeap(h);
    return;
}
void recursiveBuildHeap(H *heap, int index){
    if(index>heap->n) return; //외부 노드면 탈출
    recursiveBuildHeap(heap, index*2);
    recursiveBuildHeap(heap, index*2 + 1);
    downHeap(heap, index);
    return;
}
void downHeap(H *heap, int index){
